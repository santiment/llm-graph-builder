...
def extract_graph_from_file_local_file(graph, model, merged_file_path, fileName, allowedNodes, allowedRelationship,
    ...
    if gcs_file_cache == 'True':
        ...
        logging.info("file_name: %s pages: %s ext: %s", file_name, len(pages), file_extension)
        # NOTE: LEAK HERE??
    if not pages:
...
def upload_file(graph, model, chunk, chunk_number: int, total_chunks: int, originalname, uri, chunk_dir, merged_dir):
    ...
    if int(chunk_number) == int(total_chunks):
        ...
        return {'file_size': file_size, 'total_pages': total_pages, 'file_name': originalname,
                'file_extension': file_extension, 'message': f"Chunk {chunk_number}/{total_chunks} saved"}
