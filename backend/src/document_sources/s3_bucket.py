...
def has_supported_ext(s: str) -> bool:
    return any([s.endswith(ext) for ext in SUPPORTED_EXTENSIONS])
...
def get_s3_files_info(s3_url, aws_access_key_id=None, aws_secret_access_key=None):
    try:
        ...
        for obj in response.get('Contents', []):
            ...
            if has_supported_ext(file_name):
...
def get_s3_content(s3_url, aws_access_key_id=None, aws_secret_access_key=None):
    try:
        # Extract bucket name and directory from the S3 URL
        parsed_url = urlparse(s3_url)
        bucket_name = parsed_url.netloc
        ...
        directory = parsed_url.path.lstrip('/')
        # S3DirectoryLoader understands file extensions
        if has_supported_ext(directory):
            loader = S3DirectoryLoader(bucket_name, prefix=directory, aws_access_key_id=aws_access_key_id,
...
def get_documents_from_s3(s3_url, aws_access_key_id, aws_secret_access_key):
    try:
        ...
        pages = get_s3_content(s3_url, aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key)
